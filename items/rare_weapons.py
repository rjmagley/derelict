...
def get_rare_weapon() -> BaseWeapon:
    match random.choice(rare_weapons):
        case WeaponName.MEDIUM_IRON:
            weapon_stats = {'damage_die': 6, 'die_count': 3, 'magazine_size': 6, 'burst_count': 1, 'weapon_types': [WeaponType.PISTOL], 'name': 'Medium Iron', 'hands': 1, 'ammunition_size': 10, 'ammunition_type': AmmunitionType.HEAVY, 'minimum_range': 6, 'maximum_range': 14, 'range_interval': 5, 'accuracy_bonus': 2, 'properties': [random.choice([property_cqb, property_heavy_caliber])]}
            test_weapon = RangedPhysicalWeapon(**weapon_stats)

        case WeaponName.HOLD_THAT_THOUGHT:
            weapon_stats = {'damage_die': 4, 'die_count': 3, 'magazine_size': 20, 'burst_count': 4, 'weapon_types': [WeaponType.SMG], 'name': 'Hold That Thought', 'hands': 1, 'ammunition_size': 16, 'ammunition_type': AmmunitionType.LIGHT, 'minimum_range': 5, 'maximum_range': 15, 'range_interval': 5, 'accuracy_bonus': 0, 'properties': [random.choice([property_long_long_gun, property_light_bolt, property_belt_feeder])]}
            test_weapon = RangedPhysicalWeapon(**weapon_stats)
